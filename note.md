...
*tips：这里对一些和主题无关的部分采取了简化实现，例如：QKV三者的维度是相同的；把张量计算视为矩阵计算；头部仅仅是把原先的矩阵拆分了；没有使用FP8；使用接近原算法的e^x而不是更快的2^x*
...
4. 考虑并行性，我们可以引入分块矩阵乘法；根据分块矩阵乘法的性质，我们可以把前面讨论中的标量也视作分块矩阵。**然后我们会发现一个问题！** 在分块之前，softmax指数减去的最大值是每一行的最大值；但是分块之后，却变成了减每一个“块行”的最大值！因此我们需要进一步优化。为了方便讨论，定义 softmax<sup>*</sup> 为 softmax 的分子，也就是不做归一化处理的softmax。优化的思路基本就是，把上述的在线softmax算法改写成矩阵的形式。伪代码如下：(和上面的在线softmax的逻辑完全相同，只是改用了便于并行化的分块矩阵形式，同时把Q和K写进了算法中)
...
5. 以上内容就是FlashAttention-2的前向传播算法。可以看到，以上的过程只涉及一个Q块对所有K块的遍历（也就是把Q循环放在更外层）。这样做有一个优势，那就是我们可以并行的执行外层每一个Q块的Attention，因为Q这一层循环是可以并行化执行的，但是K这一层却只能串行处理。同理，注意力头和批次在更外层进行并行处理。（PS: **对于这里我有一个疑问**：单纯从公式来看，Q和K转置似乎是等价的。把Q循环一道外层似乎是FlashAttention-2相对第一代的一个主要改进，我想知道这究竟改进了什么？或者说关键在于为什么K这一层放在外面就不能并行处理了？）
...
2. 张量布局（Tensor Logouts）：GPU矩阵的处理方式与C相同，也就是只提供那个指向第一个元素的指针（貌似就是不能直接进行广播等高级操作？）。定义shape为形状，stride为从一个元素到这个维度上的下一个元素的距离（具体是什么，读者可以在numpy里自己试一试numpy.ndarray.shape和numpy.ndarray.strides是什么样子的）。只需要shape和stride信息（默认采用行主序布局）就可以很方便地实现reshape或者转置transpose，因为只需要改变shape和stride就可以了，不用改变内存中的元素的位置。

3. Triton: 在Triton中，每次启动只需要指定线程块，而不需要指定线程块中的具体线程。也就是说，CUDA以线程为基本单位，而Triton以块为基本单位。Triton文档有一些详细的Tutorial。Triton还有一个特点，就是它会自动把数据迁移到共享内存上（而CUDA则默认把数据放到HBM）。
